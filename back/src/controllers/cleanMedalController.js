...
exports.getAllMedals = async (req, res) => {
  try {
      ...
      const [medals] = await db.query('SELECT * FROM cleaned_olympic_medals LIMIT ?, ?', [startIndex, limit]);

      // Obtenez le nombre total d'athlètes pour calculer le nombre de pages
      const [[{ total }]] = await db.query('SELECT COUNT(*) AS total FROM cleaned_olympic_medals');
...
exports.getMedalById = async (req, res) => {
  try {
    ...
    const medal = await db.query('SELECT * FROM cleaned_olympic_medals WHERE id = ?', [medalId]);
    if (medal.length === 0) {
...
exports.MedalByCountry = async (req, res) => {
  try {
    ...
    const startIndex = (page - 1) * limit;
    const [medals] = await db.query(`
    SELECT country_name, COUNT(*) AS total_medals FROM cleaned_olympic_medals GROUP BY country_name ORDER BY total_medals DESC LIMIT ?, ?`, [startIndex, limit]);
...
exports.MedalByDiscipline = async (req, res) => {
  try {
    ...
    const startIndex = (page - 1) * limit;
    const [medals] = await db.query(`
    SELECT discipline_title, COUNT(*) AS total_medals FROM cleaned_olympic_medals GROUP BY discipline_title ORDER BY total_medals DESC LIMIT ?, ?`, [startIndex, limit]);
...
exports.MedalByAthlete = async (req, res) => {
  try {
    ...
    const startIndex = (page - 1) * limit;
    const [medals] = await db.query(`
    SELECT athlete_full_name, COUNT(*) AS total_medals FROM cleaned_olympic_medals GROUP BY athlete_full_name ORDER BY total_medals DESC LIMIT ?, ?`, [startIndex, limit]);
...
exports.getMedalsByYear = async (req, res) => {
  try {
    ...
    const [medals] = await db.query(`
      ...
      FROM cleaned_olympic_medals m
      JOIN cleaned_olympic_hosts h ON m.slug_game = h.game_slug
...
//     const result = await db.query('INSERT INTO cleaned_olympic_medals SET ?', [newMedal]);
...
//     const result = await db.query('UPDATE cleaned_olympic_medals SET ? WHERE id = ?', [updatedMedal, medalId]);
...
//   try {
//     const medalId = req.params.id;
//     const result = await db.query('DELETE FROM cleaned_olympic_medals WHERE id = ?', [medalId]);
//     if (result.affectedRows === 0) {
//       return res.status(404).json({ message: 'Medal not found' });
//     }
